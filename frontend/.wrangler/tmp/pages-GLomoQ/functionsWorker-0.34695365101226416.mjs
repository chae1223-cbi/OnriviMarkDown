...
async function executeDeleteActivations(supabaseAdmin, subscriptionIds, userIds, deviceId) {
  if (deviceId) {
...
__name(executeDeleteActivations, "executeDeleteActivations");
async function insertAuditLog(supabaseAdmin, targetUserId, adminId, actionType, reason = null) {
...
async function checkAdminAuth(request, supabaseAdmin, requiredRoles = ["SUPER", "SUPPORT"]) {
  const authHeader = request.headers.get("Authorization");
  if (!authHeader) return { error: "Unauthorized", status: 401 };
  const token = authHeader.replace("Bearer ", "");
  const { data: { user }, error: authError } = await supabaseAdmin.auth.getUser(token);
  if (authError || !user) return { error: "Unauthorized", status: 401 };
  const { data: adminData } = await supabaseAdmin.from("admins").select("admin_role").eq("user_id", user.id).single();
...
// api/admin/common-codes/groups.js
...
async function onRequestGet(context) {
  try {
    ...
    const { data: groups, error } = await supabaseAdmin.from("common_code_groups").select("*").order("sort_order", { ascending: true }).order("group_code", { ascending: true });
    if (error) throw error;
...
async function onRequestPost(context) {
  try {
    ...
    const { error: insertError } = await supabaseAdmin.from("common_code_groups").insert({
      ...
      is_use: is_use !== void 0 ? is_use : true,
      created_by: adminId,
      updated_by: adminId
    });
...
async function onRequestPatch(context) {
  try {
    ...
    const updateData = { updated_by: adminId, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
    if (group_name !== void 0) updateData.group_name = group_name;
    if (description !== void 0) updateData.description = description;
    if (sort_order !== void 0) updateData.sort_order = sort_order;
    if (is_use !== void 0) updateData.is_use = is_use;
    const { error: updateError } = await supabaseAdmin.from("common_code_groups").update(updateData).eq("group_code", group_code);
...
async function onRequestDelete(context) {
  try {
    ...
    const { error: deleteError } = await supabaseAdmin.from("common_code_groups").delete().eq("group_code", group_code);
...
function withCors(res) {
  ...
  for (const [k, v] of Object.entries(corsHeaders2)) headers.set(k, v);
...
var ROOT_ADMIN_EMAIL = "chaetang1223@gmail.com";
...
async function onRequestGet3(context) {
  try {
    ...
    const { data: logsData, error: logsError } = await supabaseAdmin.from("user_audit_logs").select("id, action_type, reason, created_at, admin_id").eq("target_user_id", userId).order("created_at", { ascending: false });
    if (logsError) throw logsError;
    const { data: codes } = await supabaseAdmin.from("common_codes").select("code_value, code_name").eq("group_code", "AUDIT_ACTION");
...
// api/admin/common-codes/index.js
var onRequestOptions6 = handleOptions;
async function checkAdmin2(supabaseAdmin, adminId, requiredRole = "SUPPORT") {
  if (!supabaseAdmin || !adminId) return false;
  const { data } = await supabaseAdmin.from("admins").select("admin_role").eq("user_id", adminId).single();
  if (!data) return false;
  if (requiredRole === "SUPER" && data.admin_role !== "SUPER") return false;
...
async function onRequestGet4(context) {
  try {
    ...
    const hasAccess = await checkAdmin2(supabaseAdmin, adminId, "SUPPORT");
    if (!hasAccess) {
      return jsonResponse({ success: false, error: "\uAD8C\uD55C\uC774 \uC5C6\uC2B5\uB2C8\uB2E4." }, 403);
    }
    let query = supabaseAdmin.from("common_codes").select("*").order("sort_order", { ascending: true }).order("code_value", { ascending: true });
    if (group_code) {
...
async function onRequestPost5(context) {
  try {
    ...
    const hasAccess = await checkAdmin2(supabaseAdmin, adminId, "SUPER");
    if (!hasAccess) {
      return jsonResponse({ success: false, error: "SUPER \uAD8C\uD55C\uC774 \uD544\uC694\uD569\uB2C8\uB2E4." }, 403);
    }
    const uppercaseCodeValue = code_value.toUpperCase().replace(/\s+/g, "");
    const { data: existingCode } = await supabaseAdmin.from("common_codes").select("id").eq("group_code", group_code).eq("code_value", uppercaseCodeValue).single();
    if (existingCode) {
...
async function onRequestPatch3(context) {
  try {
    ...
    const updateData = { updated_by: adminId, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
    if (code_name !== void 0) updateData.code_name = code_name;
    if (description !== void 0) updateData.description = description;
    if (sort_order !== void 0) updateData.sort_order = sort_order;
    if (is_use !== void 0) updateData.is_use = is_use;
    const { error: updateError } = await supabaseAdmin.from("common_codes").update(updateData).eq("id", id);
    if (updateError) throw updateError;
...
async function onRequestDelete3(context) {
  try {
    ...
    const { error: deleteError } = await supabaseAdmin.from("common_codes").delete().eq("id", id);
    if (deleteError) throw deleteError;
...
async function onRequestGet5(context) {
  try {
    ...
    const { data: inquiries, error: inquiriesError } = await supabaseAdmin.from("support_inquiries").select("*").order("created_at", { ascending: false });
    if (inquiriesError) throw inquiriesError;
    const { data: codes, error: codesError } = await supabaseAdmin.from("common_codes").select("group_code, code_value, code_name");
...
async function onRequestPatch4(context) {
  try {
    ...
    const { data: inquiryData, error: inquiryError } = await supabaseAdmin.from("support_inquiries").select("email, title, content").eq("id", id).single();
    ...
    if (answer_content !== void 0) {
      updateData.answer_content = answer_content;
      updateData.answered_at = (/* @__PURE__ */ new Date()).toISOString();
      updateData.answered_by = authResult.user.id;
      if (answer_attachment_urls !== void 0) {
        updateData.answer_attachment_urls = answer_attachment_urls;
      }
    }
    const { data: updatedInquiry, error: updateError } = await supabaseAdmin.from("support_inquiries").update(updateData).eq("id", id).select().single();
    if (updateError) throw updateError;
...
async function onRequestGet6(context) {
  try {
    ...
    const { data: plans, error: plansError } = await supabaseAdmin.from("pricing_plans").select("*").order("sort_order", { ascending: true });
...
async function onRequestPatch5(context) {
  try {
    ...
    if (!id) return jsonResponse({ error: "ID is required" }, 400);
    const { data, error } = await supabaseAdmin.from("pricing_plans").update({
      plan_code,
      sys_type,
      tagline,
      badge,
...
// api/admin/users.js
var onRequestOptions9 = handleOptions;
async function onRequestGet7(context) {
  try {
    ...
    if (type === "general") {
      ...
      let subIds = [];
      if (userIds.length > 0) {
        const { data: subs } = await supabaseAdmin.from("subscriptions").select("*").in("user_id", userIds).eq("plan_status", "ACTIVE");
        if (subs) {
          subs.forEach((s) => {
            subsMap[s.user_id] = s;
      ...
      const { data: codesData } = await supabaseAdmin.from("common_codes").select("code_value, code_name").eq("group_code", "PLAN_NAME").eq("is_use", true);
      const planCodeMap = {};
      if (codesData) {
        codesData.forEach((c) => {
      ...
      if (userIds.length > 0) {
        const actsList = [];
        const { data: actsByUser } = await supabaseAdmin.from("license_activations").select("id, created_by, subscription_id, device_name, activated_at, is_active").in("created_by", userIds).eq("is_active", true);
        if (actsByUser) actsList.push(...actsByUser);
        if (subIds.length > 0) {
          const { data: actsBySub } = await supabaseAdmin.from("license_activations").select("id, created_by, subscription_id, device_name, activated_at, is_active").in("subscription_id", subIds).eq("is_active", true);
          if (actsBySub) actsList.push(...actsBySub);
        }
        const seenActs = /* @__PURE__ */ new Set();
        actsList.forEach((a) => {
...
async function onRequestGet8(context) {
  try {
    ...
    const url = new URL(request.url);
    let rawKey = url.pathname.replace(/^\/api\/image\//, "");
    let key = rawKey;
    try {
...
__name(getOEmbedEndpoint, "getOEmbedEndpoint");
...
function withCors2(res) {
  const headers = new Headers(res.headers);
  for (const [k, v] of Object.entries(corsHeaders4)) headers.set(k, v);
  return new Response(res.body, { status: res.status, statusText: res.statusText, headers });
}
...
async function onRequestPost7(context) {
  try {
    ...
    if (!base64Data) {
      return withCors2(new Response(JSON.stringify({ error: "Bad Request: No image data provided" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      }));
    }
    ...
    if (!env.R2_BUCKET) {
      return withCors2(new Response(JSON.stringify({ error: "Internal Server Error: R2_BUCKET binding missing" }), {
        status: 500,
        headers: { "Content-Type": "application/json" }
      }));
    }
    ...
    return withCors2(new Response(JSON.stringify({
      status: "success",
    ...
    return withCors2(new Response(JSON.stringify({ error: err.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    }));
  }
...
// ../.wrangler/tmp/pages-GLomoQ/functionsRoutes-0.32867006337685634.mjs
var routes = [
  {
    routePath: "/api/admin/common-codes/groups",
    mountPath: "/api/admin/common-codes",
    method: "DELETE",
    middlewares: [],
  ...
  {
    routePath: "/api/rpc/user/check",
    mountPath: "/api/rpc/user",
    method: "OPTIONS",
    middlewares: [],
  ...
  {
    routePath: "/api/admin/common-codes",
    mountPath: "/api/admin/common-codes",
    method: "DELETE",
    middlewares: [],
...
// C:/Users/chae1/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/chae1/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// C:/Users/chae1/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/chae1/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// ../.wrangler/tmp/bundle-ksB28Q/middleware-insertion-facade.js
...
// C:/Users/chae1/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// ../.wrangler/tmp/bundle-ksB28Q/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.34695365101226416.mjs.map
