...
import { insertSupportInquiryQuery } from '@/lib/db/queries/supportQueries';
...
export async function POST(request: Request) {
  try {
    ...
    // 이메일 기반 회원 UUID 조회 (비로그인 상태 제출 시 회원 여부 판별)
    ...
    if (!resolvedUserId && p_email) {
      try {
        ...
        if (matchedUser?.id) {
          ...
          // users에 없으면 기존 users 테이블에서 이중 확인
          const { data: legacyUser } = await supabaseAdmin
            ...
            .maybeSingle();
          if (legacyUser?.id) {
            resolvedUserId = legacyUser.id;
          }
        }
      } catch (lookupErr) {
        console.warn('[/api/rpc/support/insert] 이메일 유저 조회 실패:', lookupErr);
      }
    }

    // 1. DB Insert (Primary: Supabase Client, Fallback: direct postgres sql)
    try {
      ...
      if (error) {
        console.warn('[/api/rpc/support/insert] Supabase insert failed, attempting sql fallback:', error);
        inquiryId = await insertSupportInquiryQuery(
          sql, p_name, p_email, p_type, p_title, p_content, resolvedUserId, attachmentUrls
        );
      } else {
        inquiryId = data?.id;
      }
    } catch (insertErr) {
      console.warn('[/api/rpc/support/insert] Supabase exception, attempting sql fallback:', insertErr);
      inquiryId = await insertSupportInquiryQuery(
        sql, p_name, p_email, p_type, p_title, p_content, resolvedUserId, attachmentUrls
      );
    }
    ...
    if (BREVO_API_KEY) {
      ...
      if (attachmentUrls.length > 0) {
        ...
        `;
        attachmentUrls.forEach((url: string) => {
      ...
      try {
        ...
        // We don't throw error here so that the user still gets a success message for inquiry submission
      }
    } else {
      console.warn('BREVO_API_KEY is missing. Email notification skipped.');
    }

    return NextResponse.json({ success: true, code: 'SUCCESS', message: '문의가 접수되었습니다.', inquiry_id: inquiryId });
  } catch (error: any) {
    console.error('[/api/rpc/support/insert] Error:', error);
