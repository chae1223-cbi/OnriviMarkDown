...
export default function DashboardPage() { // 🎯 @KICK : 로그인 유저 구독/라이선스/접속세션 관리 및 요금제 선택 대시보드
  ...
  const [isAnnual, setIsAnnual] = useState(false);                                     // 📆 연간/월간 요금제 토글 상태
  ...
  useEffect(() => { // ⏳ 15초 간격으로 세션 삭제 감지
    ...
    if (!sessionId || !paymentNo) return; // 🔑 결제번호가 없으면 감지 중단
    ...
    const check = async () => { // ⏳ 세션 체크 함수
      ...
      const { data: chk } = await supabase.rpc('check_license_session', { p_payment_no: paymentNo, p_device_uuid: sessionId }); // 💻 현재 브라우저 세션 ID
  ...
  // -------------------------------------------------------------------------------------
  const handleDeepLinkActivate = () => {
    if (!license || !user) { showToast('유효한 라이선스 또는 유저 정보가 존재하지 않습니다.', 'warning'); return; }
    const deepLinkUrl = `onriviauthor://activate?key=${encodeURIComponent(license.verify_key)}&user=${encodeURIComponent(user.email)}&paymentNo=${encodeURIComponent(license.payment_no)}&licenseKey=${encodeURIComponent(license.license_key)}`;
  ...
  const handleSelectPlan = async (plan: typeof plans[0]) => { // 🎯 요금제 선택
    ...
    try {
      ...
      if (plan.isFree && historyList.length > 0) { // 🛡️ 무료 요금제 재가입 방지
        throw new Error("이미 구독 신청 이력이 존재하는 계정이므로 무료 요금제 재가입이 절대 불가합니다.");
      }
      const maxDevices = plan.isFree ? 1 : (plan.name === '기본' ? 3 : plan.name === '프로' ? 6 : 9); // 🔒 최대 기기 수 설정
      const trialDays = plan.isFree ? 7 : (isAnnual ? 365 : 30); // 🛡️ 무료 체험 기간 설정
      const interval = plan.isFree ? 'trial' : (isAnnual ? 'year' : 'month'); // 🛡️ 구독 간격 설정
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px 80px" }}>
          ...
          <div style={{ ...glassCard, padding: "24px" }}>
            ...
            {license ? (
              ...
              <div style={{ padding: "12px 14px", background: "rgba(239,68,68,0.05)", border: `1px solid rgba(239,68,68,0.15)`, borderRadius: "0.75rem", marginBottom: 16, fontSize: 12, display: "flex", alignItems: "center", gap: 8, color: T.danger }}>
                ...
                활성화된 라이선스 정보가 없습니다. 요금제를 선택하여 시작해 주세요.
              </div>
            )}
            <button
              onClick={handleDeepLinkActivate}
              disabled={!license || isTrialExpired}
              style={{
                width: "100%", padding: "10px", borderRadius: "0.75rem",
                background: (!license || isTrialExpired) ? "rgba(14,165,233,0.05)" : T.primary,
                color: (!license || isTrialExpired) ? T.subtle : "#fff",
                border: `1px solid ${(!license || isTrialExpired) ? T.border : "transparent"}`,
                fontSize: 13, fontWeight: 600, cursor: (!license || isTrialExpired) ? "not-allowed" : "pointer",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                transition: "all 0.15s", opacity: (!license || isTrialExpired) ? 0.6 : 1,
              }}
            >
              <ShieldCheck size={15} /> 이 PC에서 에디터 앱 활성화하기 ↗
            </button>
          </div>
        ...
        <div style={{ ...glassCard, padding: "28px", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, paddingBottom: 16, borderBottom: `1px solid ${T.border}`, flexWrap: "wrap", gap: 12 }}>
            ...
            {/* 월간/연간 결제 선택 토글 */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(99,102,241,0.06)", padding: "4px", borderRadius: "9999px", border: `1px solid ${T.border}` }}>
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  background: !isAnnual ? T.primary : "transparent",
                  color: !isAnnual ? "#fff" : T.muted,
                  transition: "all 0.2s"
                }}
              >
                월간 결제
              ...
              >
                연간 결제
                <span style={{ fontSize: 9, background: "rgba(255,255,255,0.2)", padding: "1px 6px", borderRadius: "9999px" }}>16% 할인</span>
              </button>
            </div>
          </div>
          <div className="grid md:grid-cols-4 gap-4">
            {plans.map((plan) => {
              ...
              const isSameInterval = plan.isFree ? true : (isAnnual ? subscription?.billing_interval === 'year' : subscription?.billing_interval === 'month');
              ...
              // 연 결제 시 16% 상당 가격 할인 렌더링
              const priceVal = plan.isFree
                ? 0
                : (isAnnual ? (plan.priceMonthly! * 10) : plan.priceMonthly);
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 22, fontWeight: 800, color: T.onSurface, marginBottom: 2 }}>
                    ...
                    {!plan.isFree && <span style={{ fontSize: 12, fontWeight: 400, color: T.subtle }}>/{isAnnual ? '년' : '월'}</span>}
                  </div>
                    ...
                    return (
                      <tr key={hist.id} style={{ borderBottom: `1px solid rgba(99,102,241,0.05)` }}>
                        ...
                        <td style={{ padding: "12px 16px", color: T.muted }}>
                          {hist.billing_interval === 'year' ? '연간 결제' : hist.billing_interval === 'month' ? '월간 결제' : '무료 체험'}
                        </td>
                        <td style={{ padding: "12px 16px", color: T.muted }}>
