    ...
    <a href={href} target="_blank" rel="noopener noreferrer" className="block no-underline my-2 group">
      <span className={`block relative rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800 transition-colors ${
        ...
        <span className="aspect-video relative flex items-center justify-center bg-black/10 dark:bg-black/30 overflow-hidden">
          ...
          {!loading && !thumbnail ? (
            <span className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-zinc-200/80 to-zinc-300/80 dark:from-zinc-800/60 dark:to-zinc-900/80">
        ...
        <span className={`px-4 py-2.5 flex items-center gap-2 border-t ${
          isYoutube
            ? 'border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900'
            : 'border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900'
        }`}>
          <span className={`block w-5 h-5 shrink-0 fill-current ${isYoutube ? 'text-red-500' : 'text-zinc-400'}`}>
            {isYoutube ? (
          ...
          <span className="block text-xs font-medium text-zinc-600 dark:text-zinc-300 truncate min-w-0">
