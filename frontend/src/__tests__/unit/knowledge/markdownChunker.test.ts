...
describe('markdownChunker', () => {
  it('헤딩이 없는 서두 문단과 하위 헤딩을 올바른 계층 경로로 분할한다', () => {
    ...
    const chunks = chunkMarkdownByHeadings('doc_1', markdown, '시스템설계서');

    assert.strictEqual(chunks.length, 5);

    // 1. 서두 개요 청크 (문서명 맥락 결합)
    assert.strictEqual(chunks[0].headingTitle, '[시스템설계서] 서두 및 개요');
    assert.strictEqual(chunks[0].headingLevel, 0);
    assert.strictEqual(chunks[0].headingPath, '시스템설계서 > 서두');
    ...
    assert.strictEqual(chunks[1].headingPath, '시스템설계서 > 시스템 개요');
    ...
    assert.strictEqual(chunks[2].headingPath, '시스템설계서 > 시스템 개요 > 아키텍처');
