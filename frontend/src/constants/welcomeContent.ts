...
// 🚨 @PATCH : **2026-07-23** — 사용 설명서 텍스트 '폴더 열기'→'작업장 폴더 열기' 변경; **2026-06-25** — 웰컴페이지 전체 내용 교체
// 🔗 @CALLS : WELCOME_CONTENT, DEFAULT_WELCOME_MD
// ====================================================================
const WELCOME_MD = `# ✨ 글쓰기가 10배 즐거워지는 마법! Onrivi Author에 오신 것을 환영합니다! ✨

안녕하세요! Onrivi Author와 함께 기분 좋은 첫걸음을 내딛게 된 것을 진심으로 환영합니다! 🥰

Onrivi Author는 글을 사랑하는 작가님, 매일 문서와 씨름하는 기획자님, 그리고 나만의 지식을 멋지게 기록하고 싶은 개발자님을 위해 태어난 **문서 편집기**예요. 기존 메모장이나 워드 프로그램을 쓰면서 느꼈던 소소한 스트레스들을 싹 해결해 드리기 위해 정성껏 빚어냈답니다.

"마크다운이 뭐지? 어려우면 어쩌지?" 걱정하지 마세요! Onrivi Author가 얼마나 쉽고 다정한 친구인지, 지금부터 알기 쉽게 소개해 드릴게요. 🎈

---

## 🌟 써본 사람만 감탄하는 Onrivi Author의 '단짝 포인트 6가지'

### 1. 🔒 인터넷이 끊겨도 안심! 내 소중한 원고를 지켜요
Onrivi Author는 무겁고 불안한 외부 서버를 거치지 않고, **인터넷이 전혀 안 되는 비행기 안이나 공공기관 폐쇄망에서도 0.X초 만에 번개처럼 켜져요!** 모든 글이 내 컴퓨터(로컬 하드디스크)에만 안전하게 보관되니까, 해킹이나 원고 날아갈 걱정 없이 오직 글쓰기에만 푹 몰입하실 수 있답니다. 🛡️

### 2. ✍️ 한글 타이핑이 정말 찰지게 잘 쳐져요!
기존의 흔한 에디터들은 한글을 치다 보면 끝 글자가 씹히거나 중복으로 적히는 버그가 많아 속상하셨죠? Onrivi Author는 마이크로소프트의 VS Code 기술(Monaco Core)을 쏙 빼닮아서, **대용량 문서를 쓸 때도 글자가 밀리는 느낌 없이 물 흐르듯 매끄러운 손맛**을 자랑해요!
...
책 한 권 분량의 긴 글이나 보고서를 쓸 때 내가 지금 어디쯤 쓰고 있는지 길을 잃기 십상이죠? Onrivi Author가 문서 내의 제목 기호(\`#\`)를 실시간으로 추적하여 **오른쪽에 정갈한 탐색 목차(Table of Contents)를 자동으로 빌드**해 줍니다. 목차의 제목을 누르면 해당 본문 위치로 즉시 스크롤되어 긴 문서 작업이 대단히 쾌적해집니다.
...
1. **내 작업장 폴더 열기:** 프로그램을 켜고 상단 메뉴의 [파일] > [작업장 폴더 열기](\`Ctrl + Shift + O\`)를 눌러 글을 저장할 내 컴퓨터의 폴더를 선택해 주세요. 왼쪽 화면에 예쁜 개요 불릿(◾) 목록이 정돈됩니다.
...
*아래 구역은 사장님이 직접 튜닝하신 **\`일반_전자북_가독성_최적화_프로필_V2\`** 서식 엔진의 모든 컴포넌트 규칙이 Onrivi Author 안에서 얼마나 예쁘게 렌더링되는지 직접 타이핑하며 테스트하는 공간이에요. 마크다운 기호들을 직접 수정해 보면서 Onrivi의 명품 레이아웃을 온몸으로 느껴보세요!*
...
> "Onrivi Author는 1인 개발 기업의 한계를 아웃소싱 공급망으로 완벽히 초월하여, 초기 비용 영 원의 제로 런웨이(Zero-Runway) 해자를 방어해 냅니다."
...
👉 최신 업데이트 소식은 [Onrivi사이트](https://onrivi.com)에서 실시간으로 스캔하실 수 있습니다.
...
[![YouTube Video](https://img.youtube.com/vi/6omFnTKIC64/0.jpg)](https://www.youtube.com/watch?v=6omFnTKIC64&t=54s)
...
---
### 📌 테스트 12: 수식을 표시합니다..
Onrivi Author는 논문, 기술 기획서, 그리고 복잡한 금융 정산 아키텍처를 정의할 때 필요한 수학 공식을 아주 선명하고 아름답게 렌더링해 줘요. 전 세계 표준인 LaTeX(KaTeX) 문법을 지원하여 마우스 없이 타자만으로 기하학적인 공식 배관을 완성할 수 있답니다.
...
Onrivi Author 에디터 엔진이 렌더링해야 하는 모든 멀티미디어 컴포넌트(Yandex/Google Map 변환기용 \`[map: ]\` 가드 태그, YouTube용 \`[video: ]\` 가드 태그, 링크, 인라인 코드 가드 등)를 V2 서식 명세에 맞추어 완벽하게 심어두었습니다.
