...
const regex1 = /let\s*firstLevelFound\s*=\s*false;\s*\/\/[^\n]*\n\s*\/\/[^\n]*\n\s*\/\/[^\n]*\n\s*let\s*bufferStartEl\s*:\s*HTMLElement\s*\|\s*null\s*=\s*null;\s*for\s*\(let\s*i\s*=\s*0;\s*i\s*<\s*children\.length;\s*i\+\+\)\s*\{[\s\S]*?\/\/\s*.[^\n]*?반 콘텐/m;
...
const regex2 = /let\s*firstLevelFound\s*=\s*false;\s*const\s*extractHeadingTitle[\s\S]*?\/\/\s*.{1,3}\s*버퍼가\s*.{1,2}\s*경우/m;
