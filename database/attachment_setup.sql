...
-- 5. Brevo 이메일 트리거 업데이트 — 첨부파일 링크 포함
...
BEGIN
  ...
  IF NEW.attachment_urls IS NOT NULL AND array_length(NEW.attachment_urls, 1) > 0 THEN
    v_attachments_html := '<div style="margin-top: 15px; padding: 12px; background-color: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px;">' ||
                          '<h4 style="margin: 0 0 8px 0; font-size: 13px; color: #0369a1;">📎 첨부 파일</h4>' ||
                          '<ul style="margin: 0; padding-left: 20px; font-size: 13px;">';
    FOR i IN 1..array_length(NEW.attachment_urls, 1) LOOP
      v_attachments_html := v_attachments_html ||
        '<li><a href="' || NEW.attachment_urls[i] || '" target="_blank" style="color: #0ea5e9;">' ||
        '파일 ' || i || '</a></li>';
    END LOOP;
    v_attachments_html := v_attachments_html || '</ul></div>';
  ELSE
    v_attachments_html := '';
  END IF;
