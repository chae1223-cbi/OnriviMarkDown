...
| OMD-CORE-layout-0001 ✅ FIXED | layout | RootLayout | Next.js 루트 레이아웃 - 전역 HTML 구조, CSP, 폰트, ToastProvider 래핑 | 없음 | script defer 제거 (Next.js hydration이 제거하여 동적 로드로 전환) | ToastProvider |
...
| OMD-CORE-MarkdownViewer-0006 ✅ FIXED | MarkdownViewer | loadMermaidScript | Mermaid CDN 스크립트를 동적으로 로드하고 초기화 (SSR 번들 충돌 방지) | window.mermaid 존재 시 재사용; 중복 로딩 방지용 mermaidPromise 캐싱 | CSP 우회: 동적 script 생성(./mermaid.min.js 상대경로) + define 일시제거(AMD 충돌 회피) + fetch/eval 폴백 (Next.js hydration script 제거 + file:// path + anonymous define 3대 문제 해결) | mermaid.initialize |
