...
if (startIdx !== -1 && m) {
    ...
    const newStr = `const previewScrollTop = target.scrollTop;
                            ...
                            for (let i = 0; i < elements.length; i++) {
                              ...
                              if (elementTop > previewScrollTop) {
                                elA = i > 0 ? elements[i - 1] : elements[0];
                                break;
                              }
                              elA = elements[i];
                            }
                            ...
                            if (elA && editorRef.current) {
                              ...
                              if (lineAStr && typeof editor.getTopForLineNumber === 'function' && typeof editor.setScrollPosition === 'function') {
                                ...
                                const editorMaxScroll = editor.getScrollHeight() - viewportHeight;
                                
                                const editorTopA = lineA === 1 ? 0 : editor.getTopForLineNumber(lineA);
                                const editorTopB = elB ? editor.getTopForLineNumber(parseInt(elB.getAttribute('data-line')!, 10)) : editorMaxScroll + viewportHeight;
                                ...
                                let progress = 0;
                                if (!elB) {
                                  const remaining = previewMaxScroll - previewTopA;
                                  if (remaining > 0) progress = (previewScrollTop - previewTopA) / remaining;
                                } else if (previewTopB > previewTopA) {
                                  progress = (previewScrollTop - previewTopA) / (previewTopB - previewTopA);
                                }
                                
                                let targetEditorScroll = 0;
                                if (!elB) {
                                  targetEditorScroll = editorTopA + progress * (editorMaxScroll - editorTopA);
                                } else {
                                  targetEditorScroll = editorTopA + progress * (editorTopB - editorTopA);
                                }
