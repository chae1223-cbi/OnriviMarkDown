...
const searchTarget = 'const text = match[2].trim();\n          const lineNumber = index + 1;';
if (c.indexOf(searchTarget) !== -1) {
    ...
    console.log('Rewrote successfully using indexOf');
} else {
    // maybe it has \r\n
    const searchTargetCrLf = 'const text = match[2].trim();\r\n          const lineNumber = index + 1;';
    if (c.indexOf(searchTargetCrLf) !== -1) {
        const replacement = `let text = match[2].trim();
          ...
          const lineNumber = index + 1;`;
        c = c.replace(searchTargetCrLf, replacement);
        fs.writeFileSync(file, c, 'utf8');
        console.log('Rewrote successfully using indexOf (CRLF)');
    } else {
        console.error('Target not found at all!');
    }
}
