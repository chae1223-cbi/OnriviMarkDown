...
| **`[ONR-MD-002]`** | [MarkdownPageViewer.tsx](file:///d:/developer/OnriviMarkDown/frontend/src/components/MarkdownPageViewer.tsx) | `MarkdownPageViewer` | `MarkdownPageViewer` 함수형 컴포넌트 본체 및 페이지 분할 계산 루프 | - |
...
### `[ONR-MD-002]` 페이지 경계 및 강제 나누기 렌더링
* **구조적 위치**:
  * 컴포넌트: `MarkdownPageViewer`
  * 대상 메소드: `MarkdownPageViewer` 컴포넌트 렌더러 함수
* **내부 예외처리**:
  * 비정상적인 높이를 가진 빈 엘리먼트 렌더링 시 루프 임계 값 무한 증식을 방어하기 위해 최소 높이 보정 가드 적용.
